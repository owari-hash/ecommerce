      ...
      <div className={`py-2 transition-colors duration-300 ${transparent ? 'bg-transparent border-b border-transparent' : 'bg-white border-b border-gray-100'}`}>
        <div className="max-w-7xl mx-auto px-3 flex items-center gap-2">
          ...
          <button
            ...
            className={`sm:hidden flex-1 min-w-0 flex items-center gap-2 border rounded-lg px-3 py-2 text-sm transition-colors duration-300 ${transparent ? 'border-transparent text-white/80 bg-white/10 backdrop-blur-sm' : 'border-gray-300 text-gray-400 bg-gray-50'}`}
          >
            <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            <span className="truncate">Бараа хайх...</span>
          </button>
          ...
          <div ref={searchRef} className="hidden sm:flex flex-1 min-w-0 relative">
            <form onSubmit={handleSearch} className="flex w-full">
              <input
                ...
                className={`flex-1 min-w-0 border rounded-l-lg px-3 py-2 text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors duration-300 ${transparent ? 'bg-white/10 backdrop-blur-sm border-transparent text-white placeholder-white/70' : 'border-gray-300'}`}
              />
              <button
                type="submit"
            ...
            {user ? (
              <div className="relative hidden md:block">
                <button onClick={() => setUserMenuOpen(!userMenuOpen)} className={`flex flex-col items-center transition-colors ${transparent ? 'text-white' : 'text-primary'}`}>
                  <span className={`w-8 h-8 rounded-full overflow-hidden flex items-center justify-center ${transparent ? 'bg-white/15 ring-1 ring-white/30 backdrop-blur-sm' : 'bg-primary/10 ring-1 ring-primary/20'}`}>
                    {user.avatar ? (
                      <Image src={user.avatar} alt="" width={32} height={32} className="w-full h-full object-cover" unoptimized referrerPolicy="no-referrer" />
                    ) : (
