...
const toRGBA = (color: string, alpha = 1): string => {
    ...
    try {
        ...
        if (computed.startsWith("rgba")) return computed.replace(/[\d.]+\)$/g, `${alpha})`);
        if (computed.startsWith("rgb")) return computed.replace("rgb", "rgba").replace(")", `, ${alpha})`);
...
export const ElectroBorder: React.FC<ElectroBorderProps> = ({
    ...
    return (
        <div ref={rootRef} className={`relative isolate ${className ?? ""}`} style={style}>
            ...
            <div className="absolute inset-0 pointer-events-none z-20" style={radiusStyle}>
