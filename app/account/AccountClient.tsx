...
export default function AccountClient() {
  ...
  async function handleRegister(e: React.FormEvent) {
    e.preventDefault(); setError(''); setSuccess('');
    if (!regPhone || !regPassword) { setError('Утасны дугаар болон нууц үгээ оруулна уу'); return; }
    if (regPassword.length < 6) { setError('Нууц үг хамгийн багадаа 6 тэмдэгт байх ёстой'); return; }
    if (regPassword !== regConfirmPassword) { setError('Нууц үг таарахгүй байна'); return; }
    if (!agreeTerms) { setError('Үйлчилгээний нөхцөлийг зөвшөөрнө үү'); return; }
    setLoading(true);
    const result = await register({ phone: regPhone, email: `${regPhone}@phone.local`, password: regPassword, firstName: regPhone, lastName: '' });
    setLoading(false);
    if (result.success) {
      setUser(readAuth());
  ...
  async function handleLogout() {
    await logout(); setUser(null); setActiveTab('login');
    ...
    <div className="min-h-[70vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-black text-gray-900">{activeTab === 'login' ? 'Нэвтрэх' : 'Бүртгүүлэх'}</h1>
          <p className="text-sm text-gray-500 mt-1">{activeTab === 'login' ? 'Утасны дугаараар нэвтэрнэ үү' : 'Шинэ бүртгэл үүсгэх'}</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-xl p-7">
          <div className="flex gap-1 mb-7 bg-gray-100 rounded-xl p-1">
            <button onClick={() => { setActiveTab('login'); setError(''); setSuccess(''); setOtpSent(false); }}
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              <div className="relative">
                ...
                <input type="tel" value={regPhone} onChange={e => setRegPhone(e.target.value)}
                  className="w-full border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 bg-gray-50"
                  placeholder="Утасны дугаар" required />
              </div>
              <div className="relative">
                <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400">
                ...
                <input type="password" value={regPassword} onChange={e => setRegPassword(e.target.value)}
                  className="w-full border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 bg-gray-50"
                  placeholder="Нууц үг (6+ тэмдэгт)" minLength={6} required />
              </div>
              <div className="relative">
                <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400">
                ...
                <input type="password" value={regConfirmPassword} onChange={e => setRegConfirmPassword(e.target.value)}
                  className="w-full border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 bg-gray-50"
                  placeholder="Нууц үг давтах" required />
              </div>
